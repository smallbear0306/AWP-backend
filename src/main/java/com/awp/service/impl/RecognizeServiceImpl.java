...
public class RecognizeServiceImpl implements RecognizeService {
        ...
        try {
            ...
            String json3 = vitaClient.recognize(jpg,
                    "这是你刚才对本截图给出的记账结果 JSON：\n" + extractJson(json2) + "\n"
                            ...
                            + "3) 日期：逐位核对年/月/日，特别是年份(如 2026 不要看成 2024)；确认取的是与交易最相关的时间(交易/支付/申报时间)，若来自订单编号，核对前 8 位。\n"
                            + "4) 分类：categoryId 是否是可选分类清单里语义最贴切的那一项(结合买了什么/用途/对象)；若有更相关的二级分类请改正其 id。\n"
                            + "5) 每笔的收支方向是否正确。\n"
    ...
    private String buildExtractPrompt(List<KnowledgeCard> cards, List<Category> cats) {
        ...
        return "你是记账助手，请从这张截图提取记账所需信息。\n"
                ...
                + "【处理规则】(严格遵守)：\n" + rules
                + "【可选分类】每个二级分类后用 #数字 标注其 id，请通读全部后为每笔挑选：\n" + buildCategoryList(cats)
                + "【输出格式】严格只输出 JSON 对象 {\"items\":[ ... ]}，items 是交易数组，每个元素字段：\n"
                + "type: \"支出\" 或 \"收入\"\n"
                + "amount: 金额(数字,正数，如实填原始值，不要做加减)\n"
                + "date: 与本次交易最相关的日期(\"YYYY-MM-DD\")。优先用交易/支付/申报时间；"
                + "若页面没有明确日期，则从订单编号里提取(如形如 20260602xxxx 或 20260602-xxxx，取前 8 位作 YYYYMMDD)；"
                + "再没有才取页面上最相关的时间\n"
                + "categoryId: 必须从上面【可选分类】中选语义最相关的【一个】二级分类，返回其 # 后的数字 id。"
                + "要结合本笔交易的内容(买了什么/用途/收付对象)来匹配最贴切的一项，不要在同一一级下随意选；"
                + "尽量避免选'无法归类支出'等兜底项——实物/服务通常都能落到具体二级(如微波炉/家电→居家物业·生活用品)；"
                + "只有确实没有任何合适项时才填 null\n"
